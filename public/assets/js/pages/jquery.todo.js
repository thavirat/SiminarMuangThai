...
!function($) {
    ...
    TodoApp.prototype.generate = function() {
        ...
        for(var count=0; count<this.$todoData.length;count++) {
            ...
            var todoItem = this.$todoData[count];
            if(todoItem.done == true)
                this.$todoList.prepend('<li class="list-group-item border-0 ps-0"><div class="form-check"><input type="checkbox" class="form-check-input todo-done" id="' + todoItem.id + '" checked><label class="form-check-label" for="' + todoItem.id + '"><s>' + todoItem.text + '</s></label></div></li>');
            else {
